...
of the compiler requires the `stdlib` library to build, nothing can be built. In
...
in the commits since `trunk` -- this works with any git reference. It
...
then the generated `configure` script might differ too. Alpine Linux
Autoconf package may be used as a reasonable base to generate this
...
FROM alpine:latest
RUN --mount=type=cache,target=/etc/apk/cache apk add --update-cache autoconf
WORKDIR /mnt
