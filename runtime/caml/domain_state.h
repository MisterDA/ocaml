...
/* Alignment is necessary in caml_domain_state, since the code
 * generated by ocamlopt makes direct references into the domain state
 * structure, which is stored in a register on many platforms. For
 * this to work, we need to be able to compute the exact offset of
 * each member. */
...
typedef struct {
#define DOMAIN_STATE(type, name) alignas(8) type name;
