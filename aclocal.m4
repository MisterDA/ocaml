...
AC_DEFUN([OCAML_C99_CHECK_FMA], [
  AC_CACHE_CHECK([whether fma works], [ocaml_cv_func_fma_works], [
    OCAML_RUN_IFELSE(
      [AC_LANG_PROGRAM([[#include <math.h>]],[[
  ...
  if (!(t264 == 0x1.0989687cp-1044 ||
   ...
   || !(t266 == 0x8p-1076))
    return 1;
      ...
      [ocaml_cv_func_fma_works=yes],
      [case $enable_imprecise_c99_float_ops,$target in
        no,*)  ocaml_cv_func_fma_works=no-hard-error-true ;;
        yes,*) ocaml_cv_func_fma_works=no-hard-error-false ;;
        *,x86_64-w64-mingw32*|*,x86_64-*-cygwin)
          ocaml_cv_func_fma_works=no-hard-error-false ;;
        *) ocaml_cv_func_fma_works=no-hard-error-true ;;
      esac],
      [case $target in
        x86_64-w64-mingw32*|x86_64-*-cygwin)
           ocaml_cv_func_round_works=no-cross-compiling ;;
        *) ocaml_cv_func_round_works=yes ;;
      esac])])
  case $ocaml_cv_func_fma_works in
    yes)
      AC_DEFINE([HAS_WORKING_ROUND], [1])
      dnl OCAML_RUN_IFELSE definition of cross compiling
      if ! $host_runnable && test "x$cross_compiling" = xyes; then
    ...
    no-hard-error-true)
      AC_MSG_ERROR(m4_normalize([fma does not work, enable emulation with
        --enable-imprecise-c99-float-ops])) ;;
    no-hard-error-false)
      AC_MSG_WARN([fma does not work; emulation enabled]) ;;
    no-cross-compiling)
